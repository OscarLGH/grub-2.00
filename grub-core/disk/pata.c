...
grub_pata_open (int id, int devnum, struct grub_ata *ata)
{
  ...
  grub_pata_regset(dev, GRUB_ATA_REG_STATUS, 0x90);
  grub_pata_wait_not_busy(dev, 350000000);

  grub_pata_regset(dev, GRUB_ATA_REG_DISK, 0xE0 | (devfnd->device << 4));
  grub_pata_wait();
  grub_uint8_t lba_mid = grub_pata_regget(dev, GRUB_ATA_REG_LBAMID);
  grub_uint8_t lba_high = grub_pata_regget(dev, GRUB_ATA_REG_LBAHIGH);
